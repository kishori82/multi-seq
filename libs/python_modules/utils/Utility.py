...
def transform_refgene(refgene,ttsdis,outname):
    ...
    outf = open(outname + '.gene_anno_binexon.bed','w')
    for i in GBbin_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()        
    
    outf = open(outname+'.gene_anno_transcript.bed','w')
    for i in transcript_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()
    
    outf = open(outname+'.gene_anno_TTSdis.bed','w')
    for i in TTS400_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()

    
    outf = open(outname+'.gene_anno_5utr.bed','w')
    for i in utr5_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()
    
    outf = open(outname+'.gene_anno_3utr.bed','w')
    for i in utr3_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()

    outf = open(outname+'.gene_anno_cds.bed','w')
    for i in CDSexon_list:
        outf.write("\t".join(map(str,i))+"\n")
    outf.close()    
        
    outf = open(outname+'.gene_anno_symbol.bed','w')
...
def generate_matrix(refgene,inputbed,ttsdis,qcmatfull,qcmat,expmat,coverGNcutoff,umidis1):
    ...
    for line in inf:
        ...
        if cell == last_cell and umi == last_umi and umi != "NA" : 
            pass
        elif cell == last_cell and umi != "NA" and umidis1 == True and strdis(umi,last_umi) == 1 :
            pass
        else:
            if ttsdis == True and int(ll[11]) == 0:
                pass
            else:
